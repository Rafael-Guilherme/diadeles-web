...
export type AppPwa = 'educador' | 'responsavel';

/**
 * Corpo do service worker, comum aos dois PWAs, com estratégias de cache
 * opostas por app.
 *
 ...
 *
 * **Qual dos dois é decidido no build, por qual arquivo de entrada foi
 * compilado** (`sw.educador.ts` / `sw.responsavel.ts`), e não em tempo de
 * execução. A versão anterior deduzia isso da porta em `self.registration.
 * scope`, o que acertava em desenvolvimento e erraria em produção: com domínio
 * real, o app da família cairia no ramo do educador e passaria a guardar o dia
 * da criança em cache no aparelho.
 *
 * Duas entradas em vez de uma variável de build porque `define` do Vite não
 * chega ao service worker servido em desenvolvimento — o identificador
 * sobreviveria cru ali e o worker morreria no primeiro boot, justo no ambiente
 * onde ninguém repara.
 */
export function iniciar(app: AppPwa): void {
  ...
  if (app === 'educador') registrarCacheDoEducador();

  registrarAvisos();
  registrarSincronizacao();
}

function registrarCacheDoEducador(): void {
  const CACHE_DADOS = 'diadeles-dados-educador';
...
function registrarSincronizacao(): void {
